...
export class Datafeed {
  ...
  getBars(
    ...
    const url = `http://141.164.38.253:8090/api/history?symbol=${symbolInfo.name}&resolution=${resolution}&from=${periodParams.from}&to=${periodParams.to}`;
    fetch(url)
  ...
  subscribeBars(
    ...
    //   const url = `http://141.164.38.253:8090/api/latest?symbol=${symbolInfo.name}&resolution=${resolution}`;
    //   fetch(url)
    //     .then((response) => response.json())
    //     .then((data) => {
    //       // 새로운 데이터를 차트에 업데이트
  ...
  updateData() {
    // 서버로부터 새 데이터를 요청
    const url = `http://141.164.38.253:8090/api/latest?symbol=${this.symbol}&resolution=${this.resolution}`;
    fetch(url)
      .then((response) => response.json())
      .then((data) => {
        // 새로운 데이터를 차트에 반영
