...
export const fetchPortfolioData = createAsyncThunk(
  ...
  async (address: string) => {
    const [marketData, positions, orders, liquidityPositions, tradeHistory] =
      await Promise.all([
        axios.get("https://fix-dex.duckdns.org:8090/api/getMarket"),
        axios.get(
          `https://fix-dex.duckdns.org:8090/api/getPositions?address=${address}`
        ),
        axios.get(
          `https://fix-dex.duckdns.org:8090/api/getOrders?address=${address}`
        ),
        axios.get(
          `https://fix-dex.duckdns.org:8090/api/getLiquidityPositions?address=${address}`
        ),
        axios.get(
          `https://fix-dex.duckdns.org:8090/api/getTradeHistory?address=${address}`
        ),
      ]);
...
export const fetchMarketData = createAsyncThunk(
  ...
  async () => {
    const response = await axios.get(
      "https://fix-dex.duckdns.org:8090/api/getMarket"
    );
