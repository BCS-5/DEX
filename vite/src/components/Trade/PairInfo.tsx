...
const PairContainer: FC = () => {
  ...
  const getMarkPrice = () => {
    const url = `https://fix-dex.duckdns.org:8090/api/latest?symbol=BTC&resolution=1D`;
    fetch(url)
  ...
  const getFundingRate = () => {
    fetch("https://fix-dex.duckdns.org:8090/api/getPerpetualPool?token=BTC").then(
      (res) =>
        res.json().then((data) => {
  ...
  const getVolume = () => {
    fetch("https://fix-dex.duckdns.org:8090/api/getRecentVolume").then((res) =>
      res.json().then((data) => {
