...
const generateData = ({ timePeriod }: GenerateDataProps) => {
  ...
  return { dateArray, volumeData, feesData, tp };
};
...
const Chart: FC = () => {
  ...
  const [feesData, setFeesData] = useState<number[]>([]);
  const { tp } = generateData({ timePeriod });

  useEffect(() => {
    const fetchData = async () => {
      try {
        const From = Math.floor(Date.now() / 1000) - tp * 24 * 60 * 60;
  ...
  useEffect(() => {
    ...
    setDateArray(initialData.dateArray);
    setVolumeData(initialData.volumeData);
    setFeesData(initialData.feesData);

    // 데이터 추가 로직
    const updatedDateArray = [...initialData.dateArray];
    const updatedVolumeData = [...initialData.volumeData];
    const updatedFeesData = [...initialData.feesData];
    ...
    for (let i = 0; i < timeVolumeFeeData.length; i++) {
      ...
      updatedDateArray.push(dateString);
      updatedVolumeData.push(Number(timeVolumeFeeData[i].volume.toFixed(3)));
      updatedFeesData.push(Number(timeVolumeFeeData[i].fee.toFixed(3)));
    }
