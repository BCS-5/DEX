...
/// functions
// 초기 세팅
    function initialize(address usdtAddress, address clearingHouse) external {
        _decimals = 18;
    ...
    function setSettlementToken(address settlementToken) external onlyOwner {
        require(isContract(settlementToken), "V_CHNC");   // settlementToken is not contract
...
// 보증금 관리
    ...
    function _transferTokenIn(
        ...
        (bool success, ) = token.call(abi.encodeWithSelector(IERC20.transferFrom.selector, from, address(this), amount));
